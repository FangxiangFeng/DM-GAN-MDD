...
class FlowersTextDataset(data.Dataset):
    ...
    def load_captions(self, data_dir, filenames, class_id):
        all_captions = []
        for i in range(len(filenames)):
            cap_path = '%s/text/class_%05d/%s.txt' % (data_dir, class_id[i], filenames[i].split('/')[-1])
            # cap_path = '%s/text/%s.txt' % (data_dir, filenames[i])
            with open(cap_path, "r") as f:
                captions = f.read().decode('utf8').split('\n')
                cnt = 0
                for cap in captions:
                    ...
                    if cnt == self.embeddings_num:
                        break
                if cnt < self.embeddings_num:
                    all_captions = all_captions + [all_captions[-1]]*(self.embeddings_num-cnt)
                    print (len(all_captions), self.embeddings_num-cnt)
                    print('ERROR: the captions for %s less than %d'
                          % (filenames[i], cnt))
        return all_captions
    ...
    def load_text_data(self, data_dir, split):
        ...
        train_class_id = self.load_class_id(os.path.join(data_dir, 'train'), len(train_names))
        test_class_id = self.load_class_id(os.path.join(data_dir, 'test'), len(test_names))

        if not os.path.isfile(filepath):
            train_captions = self.load_captions(data_dir, train_names, train_class_id)
            test_captions = self.load_captions(data_dir, test_names, test_class_id)
    ...
    def __getitem__(self, index):
        ...
        data_dir = self.data_dir
        #
        img_name = '%s/flowers/%s.jpg' % (data_dir, key)
        imgs = get_imgs(img_name, self.imsize,
