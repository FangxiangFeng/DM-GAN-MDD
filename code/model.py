...
class ModalityDisentangledLayer(nn.Module):
    def __init__(self):
        super(ModalityDisentangledLayer, self).__init__()
        self.df_dim = cfg.GAN.DF_DIM
        self.nef= cfg.TEXT.EMBEDDING_DIM
        self.c_dim = cfg.GAN.CONDITION_DIM # text feature dimension
        self.z_dim = cfg.GAN.Z_DIM
        self.define_module()

    def define_module(self):
        self.ic_conv = Block3x3_leakRelu(8*self.df_dim-self.z_dim, self.c_dim)
        
    def forward(self, x):
        split_ic, split_is = x[:, :x.size(1)-self.z_dim], x[:, -self.z_dim:]
        assert split_ic.size(1) == x.size(1)-self.z_dim
        assert split_is.size(1) == self.z_dim
        split_ic = self.ic_conv(split_ic)
        assert split_ic.size(1) == self.c_dim
        x_code = torch.cat((split_ic, split_is), dim=1) # b x (256+100) x 4 x 4
        # x_code = self.reshape_conv(x_code)
        
        return x_code, split_ic, split_is
...
class D_GET_STYLE_LOGITS(nn.Module):
    def __init__(self, ndf, nef):
        super(D_GET_STYLE_LOGITS, self).__init__()
        self.df_dim = ndf
        self.ef_dim = nef
        self.uncondConv = Block3x3_leakRelu(100, 128)
        self.outlogits = nn.Sequential(
            nn.Conv2d(128, 1, kernel_size=4, stride=4),
            nn.Sigmoid())
    def forward(self, s_code):
        output = self.outlogits(self.uncondConv(s_code))
...
class D_GET_LOGITS(nn.Module):
    def __init__(self, ndf, nef, bcondition=False):
        ...
        if self.bcondition:
            self.jointConv = Block3x3_leakRelu(100+256, ndf * 8)
            self.outlogits = nn.Sequential(
                nn.Conv2d(ndf * 8, 1, kernel_size=4, stride=4),
                nn.Sigmoid())
        else:
            self.uncondConv = Block3x3_leakRelu(100+100, 128)
            self.outlogits = nn.Sequential(
                nn.Conv2d(128, 1, kernel_size=4, stride=4),
                nn.Sigmoid())
    ...
    def forward(self, h_code, c_code=None):
        if self.bcondition and c_code is not None:
            ...
            [ic_code, _] = torch.split(h_code, cfg.GAN.CONDITION_DIM, dim=1)
            h_c_code = torch.cat((ic_code, c_code), 1)
            # state size ngf x in_size x in_size
            h_c_code = self.jointConv(h_c_code)
            output = self.outlogits(h_c_code)
        else:
            output = self.outlogits(self.uncondConv(h_code))

        return output.view(-1)

class FE_NET64(nn.Module):
    def __init__(self, ndf, MD=False):
        super(FE_NET64, self).__init__()
        self.img_code_s16 = encode_image_by_16times(ndf)
        self.MD = MD
        if self.MD:
            self.style_weight = cfg.MD.BRANCH0_style
            self.content_weight = cfg.MD.BRANCH0_content
            self.modality_disentangled_layer = ModalityDisentangledLayer()
            self.style_adv_weight = cfg.MD.BRANCH0_style_adv_weight

    def forward(self, x_var): 
        x_code4 = self.img_code_s16(x_var) # 4 x 4 x 8df
        if self.MD:
            x_code, split_ic, split_is = self.modality_disentangled_layer(x_code4) # (20, 200, 4, 4)
            return x_code, torch.mean(split_ic, [2, 3]), torch.mean(split_is, [2, 3])
        return x_code4, None, None
...
class D_NET64(nn.Module):
    def __init__(self, b_jcu=True):
        ...
        ndf = cfg.GAN.DF_DIM
        nef = cfg.TEXT.EMBEDDING_DIM
        self.fe_net = FE_NET64(ndf, cfg.MD.BRANCH0)
        ...
        if self.fe_net.style_adv_weight > 0:
            self.UNCOND_S_DNET = D_GET_STYLE_LOGITS(ndf, nef)

    def forward(self, x_var):
        return self.fe_net(x_var)

class FE_NET128(nn.Module):
    def __init__(self, ndf, MD):
        super(FE_NET128, self).__init__()
        self.img_code_s16 = encode_image_by_16times(ndf)
        self.img_code_s32 = downBlock(ndf * 8, ndf * 16)
        self.img_code_s32_1 = Block3x3_leakRelu(ndf * 16, ndf * 8)
        self.MD = MD
        if self.MD:
            self.style_weight = cfg.MD.BRANCH1_style
            self.content_weight = cfg.MD.BRANCH1_content
            self.modality_disentangled_layer = ModalityDisentangledLayer()
            self.style_adv_weight = cfg.MD.BRANCH1_style_adv_weight

    def forward(self, x_var): 
        x_code8 = self.img_code_s16(x_var)   # 8 x 8 x 8df
        x_code4 = self.img_code_s32(x_code8)   # 4 x 4 x 16df
        x_code4 = self.img_code_s32_1(x_code4)  # 4 x 4 x 8df (20, 512, 4, 4)
        if self.MD:
            x_code, split_ic, split_is, ic_lc = self.modality_disentangled_layer(x_code4) # (20, 200, 4, 4)
            return x_code, torch.mean(split_ic, [2, 3]), torch.mean(split_is, [2, 3])
        return x_code4, None, None
...
class D_NET128(nn.Module):
    def __init__(self, b_jcu=True):
        ...
        ndf = cfg.GAN.DF_DIM
        nef = cfg.TEXT.EMBEDDING_DIM
        self.fe_net = FE_NET128(ndf, cfg.MD.BRANCH1)
        #
        if b_jcu:
            self.UNCOND_DNET = D_GET_LOGITS(ndf, nef, bcondition=False)
        else:
...
class FE_NET256(nn.Module):
    def __init__(self, ndf, MD, LC):
        super(FE_NET256, self).__init__()
        self.img_code_s16 = encode_image_by_16times(ndf)
        self.img_code_s32 = downBlock(ndf * 8, ndf * 16)
        ...
        self.MD = MD
        if self.MD:
            self.style_weight = cfg.MD.BRANCH2_style
            self.content_weight = cfg.MD.BRANCH2_content
            self.modality_disentangled_layer = ModalityDisentangledLayer()
            self.style_adv_weight = cfg.MD.BRANCH2_style_adv_weight
...
class D_NET256(nn.Module):
    def __init__(self, b_jcu=True):
        ...
        ndf = cfg.GAN.DF_DIM
        nef = cfg.TEXT.EMBEDDING_DIM
        self.fe_net = FE_NET256(ndf, cfg.MD.BRANCH2)
        if b_jcu:
            self.UNCOND_DNET = D_GET_LOGITS(ndf, nef, bcondition=False)
        else:
            self.UNCOND_DNET = None
