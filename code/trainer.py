...
from miscc.utils import mkdir_p, batch_tensor_to_PIL_image_grid
...
def make_directory(save_token = ''):
    ...
    save_dir = '%s/iteration%d_%s' % (s_tmp, iteration, save_token)
    os.system('mkdir -p ' + save_dir)
    return save_dir

def batch_tensor_to_image(imgs, nrow):
    '''
    converts batch of image tensor to PIL image.
    imgs: tensor, b x 3 x w x h
    nrow: it is actually ncols.
    '''
    img_set = vutils.make_grid(imgs.data, nrow = nrow).numpy()
    img_set = np.transpose(img_set, (1,2,0))
    img_set = (img_set + 1) * 255 / 2
    img_set = img_set.astype(np.uint8)
    im = Image.fromarray(img_set)
    return im

def batch_tensor_save_single_image(imgs, ncol, saver):
    bsz = imgs.size(0)
    img_set = imgs.data.numpy()
    crow = 0
    for i in range(bsz):
        if i % ncol== 0:
            crow += 1
        simg = img_set[i]
        simg = (simg+1)*255 / 2
        simg = simg.transpose((1,2,0)).astype(np.uint8)
        im = Image.fromarray(simg)
        saver(im, crow, i)

def linear_interp(x, y, npoints):
    d = (y - x) / (npoints - 1)
    interp = torch.stack([ (x + pi * d) for pi in range(npoints) ])
    return interp

def cosine_interp(x, y, npoints):
    diff = y - x
    an = [math.pi / (npoints - 1) * i for i in range(npoints)]
    inter = [(1.0 - math.cos(i)) / 2.0 for i in an]
    return torch.stack([ x + diff * it  for it in inter])
...
class condGANTrainer(object):
    ...
    def build_models(self):
        ...
        if cfg.TRAIN.NET_G != '':
            ...
            if cfg.TRAIN.B_NET_D:
                ...
                for i in range(len(netsD)):
                    s_tmp = Gname[:Gname.rfind('/')] # path
                    s_tmp2 = Gname[Gname.rfind('_'):] # _600.pth
                    Dname = '%s/netD%d_epoch%s' % (s_tmp, i, s_tmp2)
            ...
            while step < self.num_batches:
                ...
                for i in range(len(netsD)):
                    ...
                    errD, log = discriminator_loss(netsD[i], imgs[i], fake_imgs[i],
                                              sent_emb, real_labels, fake_labels, mu, noise, 
                ...
                errG_total, G_logs = \
                    generator_loss(netsD, image_encoder, fake_imgs, real_labels,
                                   words_embs, sent_emb, match_labels, cap_lens, class_ids, mu)
    ...
    def sampling(self, split_dir):
        if cfg.TRAIN.NET_G == '':
            ...
            R = np.zeros(30000)
            cont = True
            for ii in range(31):  # (cfg.TEXT.CAPTIONS_PER_IMAGE):
                if (cont == False):
                    break
